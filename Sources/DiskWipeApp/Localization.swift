...
final class Loc: ObservableObject {
    ...
    static let table: [String: [String]] = [
        ...
        // P4.5: accessibility labels (VoiceOver). Concise spoken form.
        "status.loading": ["Loading", "Загрузка", "Cargando"],
        "status.exporting": ["Exporting report", "Экспорт отчёта", "Exportando informe"],
        "common.refresh": ["Refresh", "Обновить", "Actualizar"],
        "dash.refresh.hint": ["Refreshes the disk list and reloads SMART data",
                              "Обновляет список дисков и перечитывает SMART",
                              "Actualiza la lista de discos y vuelve a leer SMART"],
        "dash.save.hint": ["Saves a Markdown, HTML and JSON report to your Downloads folder",
                           "Сохраняет отчёт Markdown / HTML / JSON в папку «Загрузки»",
                           "Guarda un informe en Markdown, HTML y JSON en Descargas"],
